...
public class EmbroideryTableMenu extends AbstractContainerMenu {
	...
	public EmbroideryTableMenu(int windowId, Inventory playerInv, ContainerLevelAccess access) {
		super(BetsyRossMenus.EMBROIDERY_TABLE_MENU.get(), windowId);
		this.access = access;
		this.container = new SimpleContainer(6);

		this.addSlot(new Slot(this.container, SPOOL_INDEX, SPOOL_SLOT_X, SPOOL_SLOT_Y) {
			@Override public boolean mayPlace(@Nonnull ItemStack stack) { return stack.is(BetsyRossTags.FLAG_MATERIAL); }
		...
		this.addSlot(new Slot(this.container, STICKS_INDEX, STICKS_SLOT_X, STICKS_SLOT_Y) {
			@Override public boolean mayPlace(@Nonnull ItemStack stack) { return stack.is(BetsyRossTags.FLAG_STICK_MATERIAL); }

            @Nullable
            @Override
            public Pair<ResourceLocation, ResourceLocation> getNoItemIcon() {
                return Pair.of(InventoryMenu.BLOCK_ATLAS, BetsyRoss.path("item/stick_no_item"));
            }
        });
	...
	@Override public boolean stillValid(Player player) { return stillValid(this.access, player, BetsyRossBlocks.EMBROIDERY_TABLE_BLOCK.get()); }
	...
	protected boolean mayPickup(Player player, TakenItem mode) {
		...
		if (!this.hasRequiredWool(this.container.getItem(SPOOL_INDEX))) return false;

		if (mode.getRequiredSticks() == 0) return true;
		ItemStack sticks = this.container.getItem(STICKS_INDEX);
	...
	public void setDataOnServer(String url, byte width, byte height) {
		...
		this.width = width;
		this.height = height;
		this.fillResultSlots(this.container.getItem(SPOOL_INDEX));
	}
	...
	public void setAndSendDataToServer(String url, byte width, byte height) {
		...
		this.width = width;
		this.height = height;
		BetsyRossClient.sendToServer(new ServerboundSyncTableDataPacket(this.url, this.width, this.height));
	}
	...
	public enum TakenItem {
		FLAG_BLOCK(() -> BetsyRossConfig.SERVER.flagBlockRequiredSticks.get(),
				...
				() -> BetsyRossItems.FLAG_ITEM.get().getDefaultInstance()),
		ARMOR_BANNER(() -> BetsyRossConfig.SERVER.armorBannerRequiredSticks.get(),
				...
				() -> BetsyRossItems.ARMOR_BANNER.get().getDefaultInstance()),
		FLAG_STANDARD(() -> BetsyRossConfig.SERVER.flagStandardRequiredSticks.get(),
				...
				() -> BetsyRossItems.FLAG_STANDARD.get().getDefaultInstance()),
		BANNER_STANDARD(() -> BetsyRossConfig.SERVER.bannerStandardRequiredSticks.get(),
				...
				() -> BetsyRossItems.BANNER_STANDARD.get().getDefaultInstance());
	...
	public class OutputSlot extends Slot {
		...
		public OutputSlot(Container itemHandler, int index, int xPosition, int yPosition, TakenItem mode) {
