...
public class BannerStandardRenderer extends BlockEntityWithoutLevelRenderer {
	...
	public void renderByItem(ItemStack stack, ItemDisplayContext transform, PoseStack posestack, MultiBufferSource buffers, int light, int overlay) {
		...
		if (transform == ItemDisplayContext.GUI) {
			...
			posestack.translate(-width * 0.5 - 0.5, 3.125, 1 / 16f);
		...
		renderFullTexture(renderInfo.location(), 0, width, height, pt, 90, posestack, buffers, light, overlay, false, FlagAnimationDetail.NO_WAVE, true);
		renderFullTexture(renderInfo.location(), 0, width, height, pt, 90, posestack, buffers, light, overlay, true, FlagAnimationDetail.NO_WAVE, true);
